    ...
    async def show_help(self, ctx, cmd: Optional[str]):
        ...
        if cmd is None:
            ...
            embed.add_field(name='🎉 **Fun**', value="`eightb`  `coinflip`  `f`  `choose`  `hug`  `rate`  `hot`  `slot`  `tableflip`  `unflip`  `calc`  `numgame`  `rps`  `riddle`  `pun`"
                ,inline=False)
            embed.add_field(name='🔧 **Utility**', value="`avatar`  `usersinfo`  `serverinfo`  `google`  `youtube`  `spotify`  `translate`  `covid`  `enlarge`  `vote`  `poll`  `instagram`  `wolframalpha`  `weather`")
            await ctx.send(embed=embed)
