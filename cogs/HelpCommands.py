...
class HelpCommands(commands.Cog):
    ...
    async def show_help(self, ctx, cmd: Optional[str]):
        ...
        if cmd is None:
            ...
            embed.add_field(name='👑 Admin', value="`kick` `ban` `unban` `mute` `unmute` `purge` `say` `edit` `showemotes` `addrole` `removerole` `move` `alladdrole` `allremoverole` `countr` `showemotes`"
                ,inline=False)
